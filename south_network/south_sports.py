...
class Scrap_News(object):
    ...
    def get_page(self):
        ...
        while index > 0:
            ...
            response = requests.get(
                "http://sports.southcn.com/mostread/default_" + str(
                    index) + ".htm",
    ...
    def get_southnetwork_url(self):
        ...
        # 保存所有可分析dom获取url的网址
        url_lists = ["http://sports.southcn.com/mostread/default.htm"]

        for i in range(2, self.page):
            url_lists.append("http://sports.southcn.com/mostread/default_" + str(i) + ".htm")
    ...
    def get_south_data(self):
        ...
        # 放之前爬过的url
        result = []  # 从文件中读取得到
        file_read = open('south_sports_url.txt', 'r')
        ...
        with open('south_sports_url.txt', "a", encoding='utf-8') as file:
            file.write(list_str + '\n')
        file.close()
