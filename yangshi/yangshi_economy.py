...
class Scrap_News(object):
    ...
    def get_page(self):
        ...
        while index > 0:
            response = requests.get(
                "https://news.cctv.com/2019/07/gaiban/cmsdatainterface/page/economy_" + str(
                    index) + ".jsonp?cb=t&cb=economy",
    ...
    def get_yangshi_urls(self):
        ...
        # 放之前爬过的url
        result = []  # 从文件中读取得到
        file_read = open('yangshi_economy_url.txt', 'r')
        ...
        with open('yangshi_economy_url.txt', "a", encoding='utf-8') as file:
            file.write(list_str + '\n')
        file.close()
    ...
    # 获取央视新闻网-经济的具体文章内容函数
    def get_yangshi_economy_news_data(self):
        url_economy = self.origin_url
        for url in url_economy:
            try:
        ...
        for index in range(len(self.titles)):
            ...
            # dict -> json(字符串类型)
            # data_json = json.dumps(data_json).encode().decode('unicode-escape')
    ...
    def start(self):
        ...
        self.get_yangshi_economy_news_data()
